...
async function loadChats() {
  ...
  try {
    ...
    CHATS_LIST = chats;
    grid.innerHTML = chats.map((c, idx) => {
      ...
      return '<div class="chat-card ' + (removed ? 'removed' : '') + '" data-chat-idx="' + idx + '">'
    ...
    grid.querySelectorAll('.chat-card[data-chat-idx]').forEach(card => {
      card.addEventListener('click', () => {
        const i = Number(card.dataset.chatIdx);
        const c = CHATS_LIST[i];
        if (c) enterChat(c);
      });
    });
  } catch (e) {
...
async function loadManage() {
  await loadChatInfo();
  // Сбрасываем чекбокс подтверждения зачистки, чтобы каждый заход в таб
  // требовал нового сознательного клика. Используем optional chaining на
  // случай если элемент ещё не отрендерен (например в standalone-режиме).
  const cp = document.getElementById('confirm-purge');
